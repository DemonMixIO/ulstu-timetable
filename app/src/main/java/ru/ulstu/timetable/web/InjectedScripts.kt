...
(function(){
  try {
    ...
    // ВАЖНО: разрыв строки даёт только <br>, поэтому сначала схлопываем переводы
    // строк из исходника — сайт пишет <BR> и перенос после него, и без этого
    // каждая строка ячейки стала бы отдельным блоком (и предмет терялся).
    function cellBlocks(cell){
      var html = (cell.innerHTML || '').replace(/\s+/g, ' ').replace(/<br\s*\/?>/gi, '\n');
